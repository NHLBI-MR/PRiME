<<<<<<< HEAD:Gen1/Components/CSaAF/Code/Launchpad/ADS1198.c
/*
 * ADS1198.c
 *
...
void ADS1198GetTimePoint(uint8_t* status,uint16_t* data)
{
	uint8_t i;
	uint8_t* dataTmp;
	//status bits are not used, but must be collected
	//delay after each TxRx are important to prevent data corruptions
	//TODO: need to check TX status bit instead of "magic" delay?
	for (i=0;i<3;i++) {
		status[i]=ADS1198SPITxRx(0);
		__delay_cycles(5);
	}
...
>>>>>>> master:Gen1/CSaAF/Code/Launchpad/ADS1198.c
